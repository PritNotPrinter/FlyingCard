'''
Lattice Boltzmann simulator for a rectangular playing card with no spin
Measurement ratios are taken from the S.T. Yau High School Award research paper at http://www.yau-awards.com/uploads/file/20210817/20210817112427_39623.pdf
'''
import jax
...
####################################################################
    #Rectangle Obstacle
    RECT_WIDTH = CYLINDER_RADIUS_INDICES * 20 # or choose a different width
    RECT_HEIGHT = CYLINDER_RADIUS_INDICES * 0.5  # or choose a different height

    obstacle_mask = (
        (X >= CYLINDER_CENTER_INDEX_X - RECT_WIDTH // 2) &
        (X <= CYLINDER_CENTER_INDEX_X + RECT_WIDTH // 2) &
        (Y >= CYLINDER_CENTER_INDEX_Y - RECT_HEIGHT // 2) &
        (Y <= CYLINDER_CENTER_INDEX_Y + RECT_HEIGHT // 2)
    )
####################################################################
    #Triangle Obstacle, point towards right
    #obstacle_mask = jnp.zeros_like(X, dtype=bool)

    #TRIANGLE_WIDTH = CYLINDER_RADIUS_INDICES * 2
    # TRIANGLE_HEIGHT = CYLINDER_RADIUS_INDICES * 3

    #  #Define bounds of triangle
    # x0 = CYLINDER_CENTER_INDEX_X
    # y0 = CYLINDER_CENTER_INDEX_Y

    # for i in range(-TRIANGLE_WIDTH // 2, TRIANGLE_WIDTH // 2):
    #     for j in range(-TRIANGLE_HEIGHT // 2, TRIANGLE_HEIGHT // 2):
    #         x = x0 + i
    #         y = y0 + j

    #         # Left-pointing triangle equation (simple linear boundary)
    #         if 0 <= x < N_POINTS_X and 0 <= y < N_POINTS_Y:
    #             if i >= 0 and abs(j) <= i * (TRIANGLE_HEIGHT / TRIANGLE_WIDTH):
    #                 obstacle_mask = obstacle_mask.at[x, y].set(True)
...
##########################################################################
    ...
    for iteration_index in tqdm(range(N_ITERATIONS)):
        ...
        if iteration_index % PLOT_EVERY_N_STEPS == 0 and VISUALIZE and iteration_index > SKIP_FIRST_N_ITERATIONS:
            ...
            #visual for the rectangle
            #plt.gca().add_patch(plt.Rectangle(
            #    (CYLINDER_CENTER_INDEX_X - RECT_WIDTH // 2, CYLINDER_CENTER_INDEX_Y - RECT_HEIGHT // 2),
            #    RECT_WIDTH,
            #    RECT_HEIGHT,
            #    color="darkgreen",
            #    )
